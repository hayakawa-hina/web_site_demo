<?php
	...
	$query4 = "select * from history where history.m_id=$1 order by history.id DESC";
	$result = pg_prepare($conn, "query4", $query4);
	$result = pg_execute($conn, "query4", array($_SESSION["id"]));
	$history_num = pg_num_rows($result);
?>
<html>
	<head>
...
<div id="page">
	...
	<div id="menu">
		...
		<div id="main">
		<br>
		<p class="form-title">閲覧履歴(20件まで)</p>
			<br>
			<div id="work" align="center">
			<table>
			<?php
			if($history_num == 0){
				print "<p class=\"regist-title\">閲覧したアルバイトはありません</p>";
			}else{
				print '<tr>';
					print '<th>仕事内容</th>';
					print '<th>時給</th>';
				...
				for($i = 0; $i < $history_num; $i++){					
					...
					print '<tr data-href=./work_detail.php?id='.$row_j["id"].'>';
